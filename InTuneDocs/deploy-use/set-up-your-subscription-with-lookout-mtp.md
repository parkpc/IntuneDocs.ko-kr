---
title: "Lookout 구독 설정 | Microsoft 문서"
...
Lookout 장치 위협 방지를 설정하는 데 다음 단계가 필요합니다.
...
Lookout Mobility Endpoint Security 테넌트가 Azure AD 구독과 연결되어 Lookout을 Intune과 통합합니다. Lookout 장치 위협 방지 서비스 구독을 사용하도록 설정하려면 Lookout 지원(enterprisesupport@lookout.com)에 다음 정보가 필요합니다.  
...
4. 이 정보를 수집했으면 Lookout 지원 팀(메일: enterprisesupport@lookout.com)에 문의합니다. Lookout 지원 팀은 주요 담당자와 함께 수집한 정보를 사용하여 구독을 등록하고 Lookout 엔터프라이즈 계정을 만듭니다.
...
>[!IMPORTANT]
> 위험 수준은 장치 위협 방지의 중요한 측면으로, 런타임 시 이러한 위험 수준에 따라 Intune 통합에서 장치 규정 준수를 측정하기 때문입니다. Intune 관리자가 장치에 최소 수준의 활성 위협(**높음**, **중간** 또는 **낮음**)이 있다면 장치가 규정을 준수하지 않는 것으로 식별하도록 정책 규칙을 설정할 수 있습니다. Lookout 장치 위협 방지의 위협 분류 정책을 통해 장치의 규정 준수 여부가 Intune에서 바로 측정됩니다.
