...
TeamViewer 통합을 제외하고, 이러한 모든 기능이 Configuration Manager와 Intune의 하이브리드 배포에 대해서도 지원됩니다. 새로운 하이브리드 기능에 대한 자세한 내용은 [Hybrid What’s New](https://technet.microsoft.com/en-us/library/mt718155.aspx)(하이브리드의 새로운 기능) 페이지를 참조하세요.
...
새로운 모든 기능에 대한 내용을 보려면 [Introducing docs.microsoft.com](https://docs.microsoft.com/teamblog/introducing-docs-microsoft-com/)(docs.microsoft.com 소개) 항목을 참조하세요.
...
- **MAM SDK: PIN 길이 구성 지원.** 장치 PIN과 유사하게 MAM 앱에 대한 PIN의 길이를 지정할 수 있게 됩니다. 이렇게 하려면, 여러분이 설정하는 새로운 제한 사항을 최종 사용자가 준수해야 합니다. 좀 더 길어진 입력을 설명하기 위해 약간 수정된 PIN 화면이 표시됩니다. 자세한 내용은 [MAM policy settings for Android](/intune/deploy-use/android-mam-policy-settings)(Android용 MAM 정책 설정) 및 [MAM policy settings for iOS](/intune/deploy-use/ios-mam-policy-settings)(iOS용 MAM 정책 설정) 항목을 참조하세요.
...
- **Windows PC에 대한 원격 지원 세션** Intune 클라이언트 소프트웨어로 관리하는 Windows PC에 TeamViewer 통합을 사용하면 Windows PC와 원격 지원 세션을 설정하여 지원 센터 부서를 지원할 수 있습니다. 지원되는 PC로는 Windows 7, 8, 8.1, Windows 10 등이 있습니다.
자세한 내용은 [Microsoft Intune 컴퓨터 클라이언트를 사용한 일반 Windows PC 관리 작업](/intune/deploy-use/common-windows-pc-management-tasks-with-the-microsoft-intune-computer-client#respond-to-a-remote-assistance-request) 항목을 참조하세요.
