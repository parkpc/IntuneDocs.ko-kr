...
title: 인증서 인프라 구성 | Microsoft Intune
description:
keywords:
author: nbigman
manager: jeffgilb
...
엔터프라이즈 인증 기관 외에도 SCEP 인증서 프로필을 사용하려면 다음이 필요합니다.

-   네트워크 장치 등록 서비스를 실행하는 서버

-   NDES를 실행하는 Windows Server 2012 R2 서버에 설치되는 Intune 인증서 커넥터

엔터프라이즈 인증 기관 외에도 .PFX 인증서 프로필을 사용하려면 다음이 필요합니다.

-  인증 기관과 통신하거나 인증 기관 컴퓨터 자체를 사용할 수 있는 컴퓨터입니다.
...
-  **NDES Server**(SCEP만 해당): Windows Server 2012 R2 이상을 실행하는 서버에 NDES(네트워크 장치 등록 서비스)를 설치해야 합니다. 엔터프라이즈 CA를 실행하는 서버에 NDES가 실행되는 경우 Intune는 사용을 지원하지 않습니다. 네트워크 장치 등록 서비스를 호스트하도록 Windows Server 2012 R2를 구성하는 방법에 대한 지침은 [네트워크 장치 등록 서비스 지침](http://technet.microsoft.com/library/hh831498.aspx)을 참조하세요.|
-  **인증 기관과 통신할 수 있는 컴퓨터**(.PFX만 해당): 또는 인증 기관 컴퓨터 자체를 사용합니다.
...
**작업 2**, SCEP 프로필에만 해당:- NDES 서버에서 필수 구성 요소 구성

**작업 3**, SCEP 프로필에만 해당 - Intune에서 사용할 NDES 구성

**작업 4** - Intune 인증서 커넥터 사용, 설치 및 구성
...
1.  NDES 서비스 계정으로 사용할 도메인 사용자 계정을 만듭니다. NDES를 설치하고 구성하기 전 발급 CA에서 템플릿을 구성할 때 이 계정을 지정합니다.
...
2.  발급 CA에서 인증서 템플릿 스냅인을 사용하여 사용자 지정 템플릿을 새로 만들거나 기존 템플릿을 복사한 다음 NDES에 사용할 수 있도록 기존 템플릿(예: 사용자 템플릿)을 편집합니다.
    ...
    -   On the **보안** 탭에서 NDES 서비스 계정을 추가하고 템플릿에 **등록** 권한을 부여합니다.
...
5.  PFX 프로필의 경우: CA 컴퓨터에서 Intune 인증서 커넥터를 호스트하는 컴퓨터에 등록 권한이 있는지 확인하여 .PFX 프로필을 만드는 데 사용된 서식 파일에 액세스할 수 있도록 합니다. CA 컴퓨터 속성의 **보안** 탭에서 권한을 설정합니다.
...
1.  NDES를 호스팅할 서버에서 **엔터프라이즈 관리자**로 로그온한 다음 [역할 및 기능 추가 마법사](https://technet.microsoft.com/library/hh831809.aspx) 를 사용하여 NDES를 설치해야 합니다.
    ...
    2.  NDES를 서버에 추가할 때는 마법사에서 IIS도 설치합니다. IIS에서 다음과 같은 구성을 사용하는지 확인합니다.
        ...
        -   **웹 서버** &gt; **응용 프로그램 배포** &gt; **ASP.NET 3.5** ASP.NET 3.5를 설치하면 .NET Framework 3.5가 설치됩니다. .NET Framework 3.5를 설치할 때는 핵심 **.NET Framework 3.5** 기능과 **HTTP 활성화**를 모두 설치합니다..

        -   **웹 서버** &gt; **응용 프로그램 배포** &gt; **ASP.NET 4.5** ASP.NET 4.5를 설치하면 .NET Framework 4.5가 설치됩니다. .NET Framework 4.5를 설치할 때는 핵심 **.NET Framework 4.5** 기능, **ASP.NET 4.5** 및 **WCF 서비스** &gt; **HTTP 활성화** 기능을 설치합니다.
...
3.  다음 명령을 실행하여 NDES 서비스 계정의 SPN을 설정합니다.

    -   **setspn -s http/&lt;NDES 서버의 DNS 이름&gt; &lt;도메인 이름&gt;\&lt;NDES 서비스 계정 이름&gt;**

    예를 들어 NDES 서버의 이름은 **Server01**, 도메인 이름은 **Contoso.com**, 서비스 계정 이름은 **NDESService**이면 다음 명령을 실행합니다.

    -   **setspn –s http/Server01.contoso.com contoso\NDESService**

## 작업 3(SCEP 프로필에만 해당) - Microsoft Intune에서 사용할 NDES 구성 
이 작업에서는 다음을 수행합니다.
...
3.  레지스트리를 편집한 후 서버에서 **iisreset** 을 실행하여 서버가 최근 구성 변경 내용을 선택하도록 강제로 설정합니다.
...
## 작업 4 - Intune 인증서 커넥터 사용, 설치 및 구성 - SCEP 및 PFX 인증서용.
이 작업에서는 다음을 수행합니다.
...
1.  [Intune 관리 콘솔](https://manage.microsoft.com)을 열고 **관리** &gt; **인증서 커넥터**를 클릭합니다..
...
1.  [Intune 관리 콘솔](https://manage.microsoft.com)을 연 다음 **관리자** &gt; **모바일 장치 관리** &gt; **인증서 커넥터** &gt; **인증서 커넥터 다운로드**를 클릭합니다..

2.  다운로드가 완료되면 다운로드한 설치 관리자(**ndesconnectorssetup.exe**)를 실행합니다.

    -   .PFX 인증서의 경우 인증 기관에 연결할 수 있는 컴퓨터에 설치 관리자를 실행합니다. .PFX 분포 옵션을 선택하고 설치를 클릭합니다. 설치가 완료되면 [인증서 프로필 구성](configure-intune-certificate-profiles.md)에서 설명한 대로 인증서 프로필을 만들어 계속합니다..

    -   SCEP 인증서의 경우 Windows Server 2012 R2 서버에서 설치 관리자를 실행합니다.

    SCEP 옵션에 대해 설치 관리자가 NDES에 대한 정책 모듈 및 CRP 웹 서비스를 설치합니다. CRP 웹 서비스 CertificateRegistrationSvc는 IIS에서 응용 프로그램으로 실행됩니다.
...
6.  명령 프롬프트를 열고 **services.msc**를 입력한 후에 **Enter** 키를 누르고 **인증서 커넥터 서비스**를 마우스 오른쪽 단추로 클릭한 다음 **다시 시작**을 클릭합니다..
