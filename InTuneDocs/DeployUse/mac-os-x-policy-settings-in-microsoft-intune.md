...
title: Microsoft Intune の Mac OS X ポリシー設定 | Microsoft Intune
description:
keywords:
author: robstackmsft
manager: jeffgilb
...
|**パスワードに必要な複雑な文字の数**|パスワードに必要な複雑な文字の数を指定します (**0** - **4**)。).<br /><br />複雑な文字とは、'**?**' などの記号です。'|
|**パスワードの最小文字数**|パスワードの最小の長さを指定します ( **4** ～ **14** 文字)。|
|**単純なパスワードを許可する**|単純なパスワードの使用を許可します ('**0000**' や '**1234**' など)。'.|
...
|設定の名前|説明|
    ...
    |**構成プロファイル ファイル**|**[インポート]**をクリックし、Apple Configurator を使用して作成した構成プロファイルを指定します。 **ヒント:** 構成プロファイルの作成については、このトピックの「[構成プロファイル ファイルを作成する方法](#BKMK_Prof)」を参照してください。|
...
-   [Apple 構成プロファイル キー参照](https://developer.apple.com/library/ios/featuredarticles/iPhoneConfigurationProfileRef/Introduction/Introduction.html)からの適切なスキーマを使用して、ファイルを自分で作成します。.
