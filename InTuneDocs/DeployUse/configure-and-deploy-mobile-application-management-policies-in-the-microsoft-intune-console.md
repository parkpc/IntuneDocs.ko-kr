...
|설정 이름|세부 정보|
    ...
    |**Android 백업 차단** 또는 **iTunes 및 iCloud 백업 차단**|앱의 정보 백업을 사용하지 않도록 설정합니다.|
    |**앱이 다른 앱으로 데이터를 전송하도록 허용**|이 앱이 데이터를 보낼 수 있는 앱을 지정합니다. 앱으로의 데이터 전송을 허용하지 않거나 관리되는 다른 앱으로만 전송을 허용하거나 어떤 앱으로든 전송하도록 허용할 수 있습니다. <br /><br />예를 들어 데이터 전송을 허용하지 않는 경우 데이터 전송은 SMS 메시지 전송, 연락처에 이미지 할당, Facebook이나 Twitter에 게시하는 것과 같은 서비스로 제한됩니다.<br /><br />iOS 장치의 경우 관리되는 앱과 관리되지 않는 앱 간의 문서 전송을 차단하려면 **관리되지 않는 다른 앱에서 관리되는 문서 허용**설정을 사용하지 않도록 설정하는 모바일 장치 보안 정책도 구성하고 배포해야 합니다. 관리되는 다른 앱으로만 전송을 허용하도록 선택하면 각 형식의 콘텐츠를 여는 데 Intune PDF 및 이미지 뷰어(배포한 경우)가 사용됩니다.<br /><br />또한 이 옵션을 **정책 관리 앱** 또는 **안 함**으로 설정하는 경우 스포트라이트 검색을 통해 앱 내의 데이터를 검색할 수 있는 iOS 9 기능이 차단됩니다.<br><br>이 설정은 모바일 장치에서 다음에서 열기 기능의 사용을 제어하지 않습니다. 여는 위치를 관리하려면[Microsoft Intune으로 iOS 앱 간의 데이터 전송 관리](manage-data-transfer-between-ios-apps-with-microsoft-intune.md)를 참조하세요.|
    |**앱이 다른 앱의 데이터를 받도록 허용**|이 앱에서 데이터를 받을 수 있는 앱을 지정합니다. 앱에서의 데이터 전송을 허용하지 않거나 관리되는 다른 앱에서만 전송을 허용하거나 어떤 앱에서든 전송하도록 허용할 수 있습니다.<br /><br />사용자가 모바일 응용 프로그램 관리 정책에 의해 관리되지 않는 앱에서 데이터에 액세스하면 데이터는 회사 데이터로 처리되며 정책에 의해 보호됩니다. 이 설정은 다중 ID를 지원하는 iOS 앱에 적용됩니다(Intune은 회사 계정 또는 앱의 데이터에만 관리 설정을 적용함). 또는 모바일 응용 프로그램 관리 정책이 적용되는 등록된 장치에 적용됩니다.|
    |**"다른 이름으로 저장" 차단**|**다른 이름으로 저장** 옵션을 사용하지 않도록 설정하여 이 정책을 사용하는 모든 앱에서 개인 클라우드 저장소 위치(예: OneDrive 또는 Dropbox)에 데이터를 저장하도록 합니다.|
    |**다른 앱에서 잘라내기, 복사 및 붙여넣기 제한**|앱에서 잘라내기, 복사 및 붙여넣기 작업을 사용할 수 있는 방법을 지정합니다. 다음 중에서 선택합니다.<br /><br />**차단됨**. 이 앱과 다른 앱 간에는 잘라내기, 복사 및 붙여넣기 작업이 허용되지 않습니다.<br /><br />**정책 관리 앱**. 이 앱과 다른 관리되는 앱 간에만 잘라내기, 복사 및 붙여넣기 작업을 허용합니다.<br /><br />**정책 관리 앱에 붙여넣기**. 이 앱에서 잘라내거나 복사한 데이터를 관리되는 다른 앱에 붙여넣는 작업만 허용합니다. 이 경우 모든 앱에서 잘라내거나 복사한 데이터를 이 앱에 붙여넣을 수 있습니다.<br /><br />**모든 앱**. 이 앱에서는 잘라내기, 복사 및 붙여넣기 작업이 제한되지 않습니다.<br /><br />관리되는 앱 간에 데이터를 복사하고 붙여넣으려면 두 앱에 모두 **정책 관리된 앱** 또는 **붙여넣기에 정책 관리된 앱** 설정이 구성되어 있어야 합니다.|
    |**액세스 시 단순 PIN 필요**|이 설정을 사용하려면 사용자가 이 앱을 사용하는 데 필요한 PIN 번호를 입력해야 합니다. 사용자가 앱을 처음 실행할 때 PIN 번호를 설정하라는 메시지가 표시됩니다.|
    |**PIN 다시 설정까지의 입력 시도 횟수**|사용자가 PIN을 다시 설정해야 하기 전까지 PIN을 입력할 수 있는 횟수를 지정합니다.|
    |**액세스 시 회사 자격 증명 필요**|사용자가 앱에 액세스하려면 이 설정에서 회사 로그온 정보를 입력해야 합니다.|
