...
# VPN connections in Microsoft Intune (Microsoft Intune での VPN 接続)
...
> デバイスにデプロイされた VPN プロファイルを使用する前に、プロファイル用の該当する VPN アプリをインストールする必要があります。 「[Microsoft Intune でアプリを展開する](deploy-apps-in-microsoft-intune.md)」の情報を参考にして、Intune を使って該当するアプリを展開してください。  
 ...
 「[VPN プロファイルのカスタム構成](custom-configurations-for-vpn-profiles.md)」では、URI 設定を使用してカスタム VPN プロファイルを作成する方法について説明されています。     
...
これは ID 証明書と呼ばれます。 ユーザーのデバイスが接続を許可されていることを示すために作成した信頼できる証明書プロファイル (または、ルート証明書) に対して認証を行うために使用されます。 信頼できる証明書は、VPN 接続を認証するコンピューター (通常は VPN サーバー) に展開されます。
...
**接続の種類**     |  VPN プロファイルで使用する接続の種類を **[Cisco AnyConnect]** (Windows 8.1 または Windows Phone 8.1 では使用できません)、**[Pulse Secure]**、**[F5 Edge Client]**、**[Dell SonicWALL Mobile Connect]**、**[CheckPoint Mobile VPN]** の中から選択します。
...
**領域**|使用する認証領域の名前を指定します。 認証領域とは、接続の種類が [Pulse Secure] の場合に使用される認証リソースのグループを表します。 このオプションは、接続の種類が **[Pulse Secure]** の場合にのみ表示されます。
...
### 関連項目
[VPN プロファイルのカスタム構成](Custom-configurations-for-VPN-profiles.md)

[Android Pulse Secure 用のアプリ別の VPN](per-app-vpn-for-android-pulse-secure.md)
