...
Windows 8.1 또는 Windows 10 데스크톱이나 모바일을 실행하는 장치의 경우에는 이전에 파일로 내보낸 Wi-Fi 구성 프로필을 가져올 수도 있습니다. 자세한 내용은, 이 항목의 뒷부분에 나오는 **Wi-Fi 프로필 가져오기**를 참조하세요.
...
|**네트워크에서 네트워크 이름(SSID)을 브로드캐스트하지 않는 경우에도 연결**|네트워크가 숨겨져 있고 네트워크 이름이 브로드캐스트되지 않아서 장치가 네트워크 목록에 표시되지 않는 경우에 장치가 네트워크에 연결되도록 하려면 이 옵션을 선택합니다.|
