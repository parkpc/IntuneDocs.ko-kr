...
3.  **검색된 소프트웨어 보고서**를 새 창으로 열려면 **보고서 보기**를 선택합니다.
    ...
    **이름**, **게시자** 또는 **범주**와 같은 각 열 제목을 선택하여 보고서를 열별로 정렬할 수 있습니다. 목록 항목 옆에 있는 방향 화살표를 클릭해서 목록에서 업데이트를 확장하여 세부 정보(예: 설치되어 있는 컴퓨터)를 표시할 수 있습니다.
...
2.  사용자를 연결할 컴퓨터를 선택한 후 **사용자 연결**을 선택합니다..
     ...
     **사용자 연결** 대화 상자에는 표시 이름과 함께 사용 가능한 사용자, 사용자 ID 및 각 사용자가 현재 연결되어 있는 컴퓨터 수의 목록이 표시됩니다. 사용자가 선택한 컴퓨터에 이미 연결되어 있는 경우 해당 사용자 이름과 사용자 ID가 **현재 사용자**아래에 표시됩니다. 컴퓨터가 어떤 사용자에게도 연결되지 않은 경우 **현재 사용자**에 **사용자 없음**이 표시됩니다..
...
## 원격 지원 요청에 응답
사용자는 관리 컴퓨터에 자동으로 설치되는 Microsoft Easy Assist를 통한 원격 지원을 사용하여 도움을 요청할 수 있습니다. 요청이 있는 경우 Intune 콘솔에 경고가 표시됩니다.

> [!IMPORTANT]
> 원격 지원은 Windows 8 이상을 실행하는 컴퓨터에서는 사용할 수 없습니다.
>
> Microsoft Easy Assist가 설치되지 않은 컴퓨터의 원격 지원 요청을 수락하면 사용자에게 Microsoft Easy Assist를 설치하라는 메시지가 표시됩니다. 설치한 후에는 컴퓨터를 다시 시작해야 합니다. 이러한 다시 시작을 방지하려면 최종 사용자의 컴퓨터에 Microsoft Easy Assist를 미리 로드하십시오.

### 원격 지원 요청을 관리하려면

1.  [Microsoft Intune 관리 콘솔](https://manage.microsoft.com/)에서 **경고** &gt; **원격 지원**을 선택합니다..

2.   **경고** 목록에서 원격 지원 요청을 선택하여 해당 요청의 속성 페이지를 엽니다.

3.  **요청을 승인하고 원격 지원을 시작합니다.**를 선택하여 경고 문제를 해결하는 옵션을 제공하는 대화 상자를 엽니다.

4.  다음 중 하나를 수행합니다.

    -   **요청 수락** - 원격 세션에 참가하려면 **원격 지원 요청 수락**을 선택합니다..

        사용자에게 **요청이 수락되었습니다. Easy Assist의 지침에 따라 프로그램 또는 데스크톱을 시스템 관리자와 공유하세요.** 라는 메시지가 표시됩니다..

        > [!IMPORTANT]
        > Intune 관리자 콘솔을 실행하는 Mac 컴퓨터에서는 원격 지원 요청을 수락할 수 없습니다.

    -   **요청 거부** - **문제 해결 정보 보기** 창을 닫은 후 경고 속성 창의 **이 경고 닫기**를 선택합니다.

        요청이 닫히며 사용자에게 요청이 거부되었다는 메시지가 표시됩니다. 사용자가 원격 지원을 다시 요청하려면 새 원격 지원 요청을 전송해야 합니다. 원격 지원 경고를 실수로 종결한 경우 원격 지원 요청을 보낸 사용자에게 새 요청을 보내도록 요청하십시오.
