...
5.  Android 6.0 이상을 사용하는 경우 이 단계를 수행합니다. 그렇지 않으면 다음 단계로 진행합니다. 
    ...
    이 메시지가 표시되면 **허용**을 탭합니다. **Microsoft는 결코 전화를 걸거나 전화 통화를 관리하지 않으므로** 허용을 탭하는 것이 안전합니다. Google이 메시지 텍스트를 제어하므로 Microsoft에서 변경할 수 없습니다. 액세스를 허용하는 경우 USB 케이블을 사용하여 로그를 이동할 수 있도록 장치에서 장치의 SD 카드에 데이터 로그를 쓸 수 있게 하는 것뿐입니다. 회사 포털 앱을 사용하는 동안 문제가 발생하면 이 기능을 사용하여 IT 관리자에 로그를 전송해야 합니다. [IT 관리자에게 등록 오류를 보내는](send-enrollment-errors-to-your-it-administrator-android.md) 방법을 알아봅니다..

    액세스를 거부하면 다음에 회사 포털에 로그인할 때 메시지가 다시 표시되지만, **다시 묻지 않음** 확인란을 탭하여 이후 메시지를 해제할 수 있습니다.  나중에 액세스를 허용할 경우 **설정** &gt; **앱** &gt; **회사 포털** &gt; **사용 권한** &gt; **휴대폰**으로 이동한 다음 사용 권한을 설정합니다.

    -   **회사 포털에 연락처에 대한 액세스를 허용하시겠습니까?**

    ![android-company-portal-sign-in](./media/and-enroll-3b-allow-contacts-access.png)

    이 메시지가 표시되면 **허용**을 탭합니다. **Microsoft는 결코 연락처에 액세스하지 않으므로** 허용을 탭하는 것이 안전합니다. Google이 메시지 텍스트를 제어하므로 Microsoft에서 변경할 수 없습니다. 액세스를 허용하는 경우 회사 포털 앱이 장치에 발생한 문제를 해결하기 위해 데이터 로그에 액세스할 수 있습니다.
...
13.  사용 중인 장치의 형식(네이티브 Android 또는 Samsung Knox)과 일치하는 다음 지침에 따릅니다. 삼성 Knox 장치가 있는지 확인하려면 **설정** &gt; **휴대폰 정보**로 이동합니다. 여기에 나열된 "Knox"라는 단어가 보이지 않으면 네이티브 Android 장치가 있어야 합니다.

    -   네이티브 (Samsung Knox가 아닌) 장치: **인증서 이름 지정** 화면에서 **확인**을 탭하여 기본 인증서를 적용합니다.

    ![인증서 지정 화면](./media/and-enroll-7-cert-native.png)

    -   Samsung Knox 장치: 개인 정보 취급 방침에 동의하고 **확인**을 탭합니다..
...
회사 앱을 설치하기 전에 **설정** &gt; **보안**으로 이동하여 **알 수 없는 소스**를 설정합니다. 앱을 설치하기 전에 이 옵션을 설정하지 않으면 "설치가 차단되었습니다. 보안상의 이유로 휴대폰이 알 수 없는 소스에서 가져온 앱의 설치를 차단하도록 설정되었습니다."라는 메시지가 표시됩니다. 오류 대화 상자에서 **설정**을 탭하여 **알 수 없는 소스** 옵션으로 이동할 수 있습니다.
