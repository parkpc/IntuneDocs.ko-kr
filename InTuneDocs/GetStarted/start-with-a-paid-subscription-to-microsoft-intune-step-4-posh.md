...
# PowerShell을 사용하여 Intune 라이선스 관리
사용자가 로그인하여 Intune 서비스를 사용하거나 장치를 관리되는 상태로 등록하려면 먼저 [Intune 라이선스 관리](start-with-a-paid-subscription-to-microsoft-intune-step-4.md)에 각 사용자에게 Intune 구독에 사용할 라이선스를 할당해야 합니다. 그러나 Microsoft의 EMS(Enterprise Mobility Suite)를 사용하는 조직에는 EMS 패키지의 Intune 서비스나 Azure Active Directory Premium만 필요한 사용자가 있을 수 있습니다. [Azure Active Directory PowerShell cmdlet](https://msdn.microsoft.com/library/jj151815.aspx)을 사용하여 한 가지 서비스 또는 서비스 하위 집합을 할당할 수 있습니다. 
...
### 다음 단계
축하합니다. *Intune quick start guide(Intune 빠른 시작 가이드)*의 4단계를 완료했습니다.
