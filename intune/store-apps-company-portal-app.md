...
---
# <a name="manually-add-the-windows-10-company-portal-app-using-microsoft-intune"></a>Microsoft Intune을 사용하여 Windows 10 회사 포털 앱 수동으로 추가
...
1. [Microsoft Store for Business](https://www.microsoft.com/business-store)로 이동하여 관리자 계정으로 로그인되도록 합니다.
2. 창 위쪽 부근에 있는 **관리** 탭을 선택합니다.
3. 왼쪽 탐색 열에서 **설정**을 선택합니다.
4. **쇼핑 경험** 섹션에서 **오프라인 앱 표시**를 **On**으로 설정합니다. 이는 Microsoft Store for Business에서 오프라인 사용이 허가된 앱을 표시합니다.

## <a name="download-the-offline-company-portal-app"></a>오프라인 회사 포털 앱 다운로드
1. **회사 포털** 앱을 검색하고 선택합니다.
2. **라이선스 유형**을 **오프라인**으로 설정합니다.
3. **앱 가져오기**를 클리하여 오프라인 회사 포털 앱을 가져와 인벤토리에 추가 합니다.
4. **회사 포털** 앱 페이지에서 **관리**를 클릭합니다.
5. **플랫폼**으로 **Windows 10 모든 장치**를 선택한 다음, 적절한 **최소 버전**, **아키텍처** 및 앱 메타데이터** 값 다운로드를 선택합니다. 
6. **다운로드**를 클릭하여 로컬 컴퓨터에 파일을 저장합니다.

    ![Windows 10 모든 장치 및 아키텍처 X86 패키지 다운로드 세부 정보를 보여 주는 이미지](./media/Win10CP-all-devices.png)

7. "Required Frameworks"(필요한 프레임워크)에 나온 모든 패키지를 다운로드합니다. x86, x64 및 ARM 아키텍처에 대해 수행해야 하므로 총 12개의 패키지가 됩니다.
8. 회사 포털 앱을 Intune에 업로드하기 전에 패키지를 포함하는 폴더(예: C:&#92;Company Portal)를 다음과 같은 방법으로 만듭니다.
   ...
   - *Dependencies* 폴더에 종속성 패키지를 배치합니다. 
     ...
     > [!NOTE]
     > 종속성을 올바른 형식으로 배치하지 않는 경우 Intune은 패키지 업로드 도중에 파일을 인식 및 업로드할 수 없습니다. 이에 따라 업로드에 실패하고 오류가 표시됩니다.

9. Azure Portal의 Microsoft Intune에 반환합니다.
10. 회사 포털 앱을 새 앱으로 업로드합니다. 원하는 대상 사용자 집합에게 필수 앱으로 할당합니다.  
...
1. [https://aka.ms/win10cpscript](https://aka.ms/win10cpscript)에서 Microsoft Intune Windows 10 회사 포털 앱 서명 스크립트를 다운로드합니다.  이 스크립트가 작동하려면 호스트 컴퓨터에 Windows 10용 Windows SDK가 설치되어야 있어야 합니다. Windows 10용 Windows SDK를 다운로드하려면 [https://go.microsoft.com/fwlink/?LinkId=619296](https://go.microsoft.com/fwlink/?LinkId=619296)을 참조하세요.
