...
7. **설정** 블레이드에서 다음 정보를 구성합니다.
    - **서비스 분기**: 장치에서 Windows 업데이트를 수신하는 분기(현재 분기 또는 비즈니스용 현재 분기)를 설정합니다.
    ...
    품질 업데이트는 일반적으로 기존 Windows 기능에 대한 수정 및 개선 사항이며 대체로 매월 첫 번째 화요일에 게시되지만 Microsoft의 재량에 따라 언제든지 릴리스될 수 있습니다. 출시된 후에 품질 업데이트 수신을 연기할지 여부 및 연기할 기간을 정의할 수 있습니다.
    - **기능 업데이트 지연 기간(일)**: 기능 업데이트가 지연되는 일 수를 지정합니다. 이러한 기능 업데이트는 출시되고 180일 동안 수신을 연기할 수 있습니다.
