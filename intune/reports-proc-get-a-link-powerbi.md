...
1.  보고 블레이드에서 **OData URL**을 검색합니다. 예: `https://fef.{yourinfo}.manage.microsoft.com/ReportingService/DataWarehouseFEService/dates?api-version=beta`.
2.  **Power BI Desktop**을 엽니다.
3.  **홈** > **데이터 가져오기**를 선택합니다. **OData 피드**를 선택합니다.
4.  **기본**을 선택합니다.
5.  **OData URL**을 URL 상자에 입력하거나 붙여넣습니다.
6.  **확인**을 클릭합니다.
7.  Power BI 데스크톱 클라이언트에서 테넌트에 대해 Azure AD 인증을 받지 않은 경우 자격 증명을 입력합니다.  
    a.  **조직 계정**을 선택합니다.  
    b.  사용자 이름과 암호를 입력합니다.  
    c.  **로그인**을 클릭합니다.  
    d.  **연결**을 클릭합니다.  
