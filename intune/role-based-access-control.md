...
새로운 Intune 포털부터는 **Azure AD(Azure Active Directory)**에서 Intune에 사용할 수 있는 두 가지 디렉터리 역할을 제공합니다. 이러한 역할에는 Intune에서 모든 활동을 수행할 수 있는 모든 권한이 부여됩니다.
...
## <a name="roles-created-in-the-intune-classic-console"></a>Intune 클래식 콘솔에서 만든 역할

"모든" 권한을 소유한 Intune **서비스 관리자** 사용자만이 Intune 클래식 콘솔에서 Azure의 Intune으로 마이그레이션됩니다. 따라서 "읽기 전용" 또는 "기술 지원팀" 액세스 권한이 있는 Intune **서비스 관리자** 사용자는 Azure Portal의 Intune 역할로 다시 할당한 다음 클래식 포털에서 제거해야 합니다.

> [!IMPORTANT]
> 관리자가 Intune을 사용한 PC 관리 기능에 계속 액세스해야 하는 경우에는 클래식 콘솔에서 Intune 서비스 관리자 액세스 권한을 유지해야 할 수도 있습니다.
